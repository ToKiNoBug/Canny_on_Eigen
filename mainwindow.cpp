...
void MainWindow::on_DoubleT_clicked()
{
    //if(Canny->step<3)return;
    /*int a,b,c;
    Canny->getThreshold(a,b,c);*/
    Canny->ApplyDoubleThreshold(20.0f/255,50.0f/255);
    //Canny->ApplyOTSUThreshold();
    DoubleThresholdMap=QImage(Canny->Height,Canny->Width,QImage::Format_Grayscale8);
    //int maxI=Canny->EI.array().maxCoeff();
    qDebug("开始生成边缘识别结果");
    for(int r=0;r<Canny->Height;r++)
    {
        unsigned char*sL=DoubleThresholdMap.scanLine(r);
        for(int c=0;c<Canny->Width;c++)
        {
            //qDebug()<<"r="<<r<<";  c="<<c<<";";
            sL[c]=255-((Canny->Edges(r+1,c+1)>0)?255:0);
        }

    }
    ui->ShowDoubeT->setPixmap(QPixmap::fromImage(DoubleThresholdMap));
    //Canny->step=5;
}
